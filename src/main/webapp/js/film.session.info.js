...
function drawSeats(allTickets) {
    ...
    allTickets.forEach(function(item, i, arr) {
        ...
        rsl += '<p>';
        rsl += '<input class="js-seat-checkbox" type="checkbox" id="seat' + String(item.row) + String(item.seat) + '" data-row="' + item.row + '" data-seat="' + item.seat + '" data-price="' + item.price + '" ' + (item.accountID !== 0 ? 'disabled' : '') + '>';
